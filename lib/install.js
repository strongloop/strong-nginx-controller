...
function printHelp($0, prn) {
  var usageFile = require.resolve('../bin/sl-arc-lb-install.txt');
...
function install(argv, callback) {
  ...
  var jobConfig = {
    user: 'strong-arc-lb',
    lbBaseDir: '.strong-arc-lb', // this should be options.cwd from fillInHome
    lbControl: 'http://0.0.0.0:0',
    lbListenPort: 'http://0.0.0.0:8080',
    dryRun: false,
  ...
  while ((option = parser.getopt()) !== undefined) {
    switch (option.option) {
      ...
      case 'h':
        ...
        return callback();
      case 'b':
        jobConfig.lbBaseDir = option.optarg;
        break;
      case 'c':
        jobConfig.lbControl = option.optarg;
        break;
      case 'l':
        jobConfig.lbListenPort = option.optarg;
        break;
      case 'u':
...
function setCommand(options, callback) {
  options.name = 'strong-arc-lb';
  options.lbBaseDir = path.resolve(options.cwd, options.lbBaseDir);
  options.command = [
    process.execPath,
    require.resolve('../bin/sl-arc-lb'),
    '--control', options.lbControl,
    '--listen', options.lbListenPort,
    '--base', options.lbBaseDir
  ];
  // always async
...
function ensureBaseDir(options, callback) {
  if (options.dryRun) {
    console.log('would create', options.lbBaseDir);
    return setImmediate(callback);
  }
  mkdirp(options.lbBaseDir, {}, function(err, made) {
    if (err) {
...
function ensureOwner(options, callback) {
  if (options.dryRun) {
    console.log('Would chown ', options.lbBaseDir);
    return setImmediate(callback);
  }
  console.error('ensuring owner: ', options.lbBaseDir);
  uidNumber(options.user, options.group, function(err, uid, gid) {
    ...
    subPaths(options.lbBaseDir, function(err, paths) {
