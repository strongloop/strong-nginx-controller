...
var MAX_DELAY = 1000 * 1000; // 1sec
var reloadTimeoutHandle;
var delayStartTime;
...
function reload(callback) {
  ...
  if (reloadTimeoutHandle) {
    if ((+Date.now() - delayStartTime) > MAX_DELAY) {
      return setImmediate(callback);
    }

    debug('Another reload requested. Deferring reload...');
    clearTimeout(reloadTimeoutHandle);
  } else {
    debug('Nginx reload requested. Waiting a little to avoid multiple reloads');
    delayStartTime = +Date.now();
  }

  reloadTimeoutHandle = setTimeout(function() {
    debug('reloading...');
    reloadTimeoutHandle = null;
    self._reload(function(err) {
      if (err)
        console.error('Error reloading Nginx configuration: %s', err.message);
    });
  }, 500);
